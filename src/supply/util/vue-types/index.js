import { isPlainObject, isArray, isInteger, isDef, isFunction } from '../index'
import { toType, validateType, getType } from './util'
...
const VuePropTypes = {
    ...
    get func () {
      return toType('function', {
        type: Function,
      }).def(currentDefaults.func) // this.default = 
    },
    ...
    oneOf (arr) {
      if (!isArray(arr)) { 
        throw new TypeError('[VueTypes error]: You must provide an array as argument | 传入的必须是一个数组')
      }
      ...
      return toType('oneOf', {
        ...
        validator (value) {
          ...
          const valid = arr.indexOf(value) !== -1 // 若含有传入的值
          if (!valid) warn(msg)
          return valid
        },
      })
