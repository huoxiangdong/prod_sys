export class draw {
    ...
    line(lines) { // 画线
        ...
        while(lines.length) {
         ...
         const p = this.pointCalc({ // end point
            x: s[0],
            y: s[1],
            r: l ? l : e[0] - s[0],
            ang: angle ? angle : 0
    ...
    pointCalc({ // 计算圆上任一点
        ...
        return {
            x: x + Math.cos(ang ? this.radian(ang) : Math.PI * 2) * r,
            y: y + Math.sin(ang ? this.radian(ang) : Math.PI * 2) * r,
            r: Math.sqrt(Math.pow(r, 2) * 2 * (1 - Math.cos(ang ? this.radian(ang) : Math.PI * 2))) / 2
        }
    }
   radian(angle) { // 角度转弧度
