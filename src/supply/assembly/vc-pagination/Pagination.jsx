...
export default {
    ...
    props: {
      ...
      defaultCurrent: PROPTYPES.number.def(1), // 默认的当前页数
      total: PROPTYPES.number.def(0),
      pageSize: PROPTYPES.number, // 每页条数
      defaultPageSize: PROPTYPES.number.def(10),
      change: PROPTYPES.func.def(noop),
      hideOnSinglePage: PROPTYPES.bool.def(false), // 只有一页时是否隐藏分页器
    ...
    data () {
      ...
      if (hasCurrent && !hasOnChange) {
        warn('Warning: You provided a `current` prop to a Pagination component without an `onChange` handler. This will render a read-only component.'); // eslint-disable-line
      }
    ...
    methods: {
      ...
      handleGoTO (event) {
        if (event.keyCode === KEYCODE.ENTER || event.type === 'click') { // enter or click
