...
export default {
  ...
  methods: {
    ...
    renderRowSelection (locale) {
      ...
      if (rowSelection) { // 列表项是否可选择
        const data = this.getFlatCurrentPageData().filter((item, index) => { // flat 平铺 body
          if (rowSelection.getCheckboxProps) { // 选择框的默认属性配置
        ...
        const selectionColumnClass = classNames(`${prefixCls}-selection-column`, {
          [`${prefixCls}-selection-column-custom`]: rowSelection.selections, // 自定义选择项
        })

        const selectionColumn = {
          key: 'selection-column',
          // renderSelectionBox -> return fn
          customRender: this.renderSelectionBox(rowSelection.type), // type 多选/单选，checkbox or radio
    ...
    // 当前页
    getMaxCurrent (total) {
      const { current, pageSize } = this.sPagination // 当前页 ，页数据大小
      // ?
    ...
    renderPagination (paginationPosition) {
      ...
      const total = pagination.total || this.getLocalData().length // 数据总数
    ...
    // 分页 排序 过滤
    getCurrentPageData () {
      let data = this.getLocalData() // 经 排序 过滤
      let current
      let pageSize
      const sPagination = this.sPagination
      // 如果没有分页的话，默认全部展示
      if (!this.hasPagination()) {
        pageSize = Number.MAX_VALUE // 最大值
        current = 1 // 当前页
      } else {
        pageSize = sPagination.pageSize // 每页显示多少数据
        current = this.getMaxCurrent(sPagination.total || data.length) // 当前页
      }
    ...
    getFlatCurrentPageData () {
      return flatArray(this.getCurrentPageData()) // 传入当前页显示数据
    },
    // 排序 递归
    ...
    getLocalData () { // 过滤 排序
      ...
      // 优化本地排序
      data = data.slice(0)
      const sorterFn = this.getSorterFn() // 排序 func
      if (sorterFn) {
      ...
      if (filters) {
        Object.keys(filters).forEach((columnKey) => {
          const col = this.findColumn(columnKey) // 根据key寻找到 col
          if (!col) {
            return
          }
    ...
    renderTable  (contextLocale, loading) {
      const locale = { ...contextLocale, ...this.locale } // 语言
      
      const { prefixCls, showHeader, ...restProps } = getOptionProps(this)

      const data = this.getCurrentPageData() // 当前页数据 -> 排序 过滤 分页
      const expandIconAsCell = this.expandedRowRender && this.expandIconAsCell !== false
      
      // class
      const classString = classNames({
        [`${prefixCls}-${this.size}`]: true,
        [`${prefixCls}-bordered`]: this.bordered, // 边框
        [`${prefixCls}-empty`]: !data.length, // 空文本
        [`${prefixCls}-without-column-header`]: !showHeader, // 不显示header
      })
